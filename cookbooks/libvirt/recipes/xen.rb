...
case node[:platform]
when "debian"
  %w{ dnsmasq-base libdevmapper-dev libgnutls-dev libxen-dev libxml2-dev python-dev }.each do |pkg|
    package pkg
  end
end
...
bash "Install libvirt #{node[:libvirt][:src_version]}" do
  ...
  cd libvirt-#{node[:libvirt][:src_version]} && ./configure --prefix=/usr/local --with-xen --with-python
